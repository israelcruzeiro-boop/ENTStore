...
import { ArrowLeft, Plus, Image as ImageIcon, Edit2, Trash2, Play, FileText, Link as LinkIcon, File, CheckCircle2, XCircle, List, ExternalLink, Calendar, Search, ArrowDownUp, PlayCircle, FileSpreadsheet, Presentation, Folder, Link2, Eye, Users, Star, Layers, ArrowUp, ArrowDown, Loader2, Music } from 'lucide-react';
...
export const AdminRepositoryContents = () => {
  ...
  const { contentViews, contentRatings, isLoading: loadingMetrics } = useRepositoryMetrics(repoId);
  ...
  const [formData, setFormData] = useState({
    title: '', description: '', thumbnail_url: '', type: 'VIDEO' as Content['type'], url: '', embed_url: '', featured: false, recent: true, status: 'ACTIVE' as 'ACTIVE' | 'DRAFT', category_id: ''
  });
  ...
  const handleCloseForm = () => {
    ...
    setTimeout(() => {
      setEditingId(null);
      setFormData({ title: '', description: '', thumbnail_url: '', type: 'VIDEO', url: '', embed_url: '', featured: false, recent: true, status: 'ACTIVE', category_id: '' });
      setBatchLinks([]);
    }, 300);
  };
  ...
  const openCreateFull = () => {
    setEditingId(null);
    setFormData({ title: '', description: '', thumbnail_url: '', type: isPlaylist ? 'MUSIC' : isVideoPlaylist ? 'VIDEO' : 'VIDEO', url: '', embed_url: '', featured: false, recent: true, status: 'ACTIVE', category_id: '' });
    setIsFormOpen(true);
  };

  const openEditFull = (content: Content) => {
    setEditingId(content.id);
    setFormData({ title: content.title, description: content.description, thumbnail_url: content.thumbnail_url, type: content.type, url: content.url, embed_url: content.embed_url || '', featured: content.featured, recent: content.recent, status: content.status || 'ACTIVE', category_id: content.category_id || '' });
    setIsFormOpen(true);
  };
  ...
  const handleSaveSimple = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingId) {
        ...
        if (originalLink) {
          const { error } = await supabase.from('simple_links').update({ name: originalLink.name, url: originalLink.url, type: originalLink.type || 'Link' }).eq('id', editingId);
          if (error) throw error;
        }
  ...
  return (
    <div className="max-w-6xl mx-auto pb-12">
      <Link to={`/admin/${link_name}/repos`} className="inline-flex items-center gap-2 text-sm text-indigo-600 hover:text-indigo-800 font-medium mb-6">
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden mb-8 relative">
         ...
         <div className="p-6 relative -mt-16 flex flex-col md:flex-row items-start md:items-end justify-between gap-4">
            ...
            <div className="bg-white px-5 py-3 rounded-xl border border-slate-200 shadow-sm flex flex-col items-center">
               <span className="text-xs font-bold text-slate-500 uppercase tracking-wider">{isSimple ? 'Total de Links' : isPlaylist ? 'Total de Músicas' : isVideoPlaylist ? 'Total de Vídeos' : 'Total de Conteúdos'}</span>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
         <h2 className="text-xl font-bold text-slate-900">{isSimple ? 'Lista de Links' : isPlaylist ? 'Faixas da Playlist' : isVideoPlaylist ? 'Vídeos da Playlist' : 'Conteúdos do Repositório'}</h2>
            ...
            <form onSubmit={handleSaveFull} className="space-y-4 mt-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                 <div className="space-y-2 sm:col-span-2">
                   ...
                   <Input placeholder="https://..." value={formData.url} onChange={(e) => setFormData({...formData, url: e.target.value})} disabled={isSubmitting} />
                 </div>
                 <div className="space-y-2 sm:col-span-2">
                  ...
                  {batchLinks.map((link, index) => (
                     <div key={link.id} className="grid grid-cols-[2fr_3fr_1.5fr_auto] gap-3 items-center group">
                        ...
                        <Button 
                          ...
                          disabled={batchLinks.length === 1 && !editingId || isSubmitting} 
