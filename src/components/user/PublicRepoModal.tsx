...
import { Repository, Theme, ContentType } from '../../types';
import { X, PlaySquare, FileText, Music, Image as ImageIcon, ExternalLink, ShieldAlert } from 'lucide-react';
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 md:p-6 lg:p-12">
          ...
          >
            ...
            >
                  ...
                  <div className="space-y-4">
                      ...
                      {contents.length > 0 && (
                          <div className="space-y-3">
                              {contents.map(item => (
                                  <a 
                                      href={getDirectUrl(item.url)} 
                                      target="_blank" 
                                      rel="noopener noreferrer" 
                                      key={item.id} 
                                      className="group relative flex items-center gap-4 p-4 rounded-xl bg-white/5 border border-white/5 transition-colors hover:bg-white/10"
                                  >
                                      ...
                                      <div className="flex-1 min-w-0">
                                          <h4 className="text-white font-bold text-base md:text-lg truncate group-hover:text-transparent group-hover:bg-clip-text transition-all duration-300" style={{ backgroundImage: `linear-gradient(to right, ${theme.text}, ${theme.primary})` }}>
                                              {item.title}
                                          </h4>
                              ...
                              {simpleLinks.map(link => (
                                  ...
                                  >
                                      ...
                                      <div className="flex-1 min-w-0">
                                          ...
                                          {link.type && <p className="text-sm text-zinc-400 truncate mt-0.5">{link.type}</p>}
                                      </div>
                                      <div className="opacity-0 group-hover:opacity-100 transition-opacity">
                                          <ExternalLink size={20} className="text-zinc-500 group-hover:text-white transition-colors" />
                                      </div>
              ...
              <div className="w-full flex justify-center p-6 bg-black/50 border-t border-white/10">
                 <button className="w-full md:w-auto px-10 py-4 rounded-xl font-bold text-lg text-white shadow-xl transition-transform hover:scale-105" style={{ backgroundColor: theme.primary }}>
                    Cadastrar e Acessar a Plataforma Completa
                 </button>
              </div>
